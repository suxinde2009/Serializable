...
class CustomOperatorsTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            if let path = Bundle(for: self.dynamicType).pathForResource("CustomOperatorsTest", ofType: "json"), data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
    ...
    func testCustomOperatorsEncode() {
		...
		XCTAssertEqual(CustomOperatorsTestNestedModel.array(rep.value(forKey: "some_array")), subArray, "Array encode with custom operator failed.")
    }
